...
public class EasyPlayScreen extends PlayScreen {

	public EasyPlayScreen(Game g, int lives, boolean twoP) {
		super(g, lives, twoP);
		createCards();
	}
	...
	protected void update(float delta) {
		...
		if (!gameOver) {
			...
			if (!twoPlayer) {
				if (lives < 1) {
					loser = true;
					gameOver = true;
					tilePostGame.setText(font, "Tiles matched: " + completedTiles + " / 12");
				}
			}
		...
		// If the game is over
			...
			if (replayBtn.getBoundingRectangle().contains(mousePos.x, mousePos.y)) {
				...
				if (Gdx.input.justTouched()) {
					game.setScreen(new EasyPlayScreen(game, 20, twoPlayer));
				}
			}
			else {
	...
	protected void createCards() {
		...
		for (int i = 0; i < 12; i++) {
			...
			if (i < 4)
				cards[i] = new Card(206 + (i * 153), 473, cardColor);
			else if (i >= 4 && i < 8)
				cards[i] = new Card(206 + ((i - 4) * 153), 320, cardColor);
			else
				cards[i] = new Card(206 + ((i - 8) * 153), 167, cardColor);
		}
	}
}
